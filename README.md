...
Telecom companies lose a large share of revenue to **churn** — customers cancelling their service. Because winning a new customer costs far more than keeping an existing one, even a small reduction in churn has an outsized impact on profit. This project answers one question: **which customers are about to leave, while there's still time to keep them?**
...
An interactive **Streamlit app** ([`app.py`](app.py)) lets you enter a customer's details and get a live churn-risk prediction from the trained model, reusing the same tested preprocessing pipeline. See [How to Run](#how-to-run) below to launch it locally.
...
All charts below are generated by the notebooks and saved to `outputs/figures/`.
